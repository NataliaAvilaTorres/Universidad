...
void Grafo::costoConquista(int id_terriObjetivo, int id_jugador)
{
    ...
    if(id_terriAtacante != -1)
    {
        cout<<"Atacando desde el territorio "<<id_terriAtacante<<" al territorio "<<id_terriObjetivo<<endl;
        //Llamar a la funcion atacar (id_terriAtacante, id_terriObjetivo)
    }
    else
    {
        cout<<"El jugador "<<id_jugador<<" no controla ningun territorio cercano al objetivo."<<endl;
    }
}
