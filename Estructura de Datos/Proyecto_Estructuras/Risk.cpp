  ...
  else
  {
    ...
    for(list<Jugador>::iterator it = jugadores.begin(); it != jugadores.end(); it++)
    {
        if ((*it).getColor_jugador() == idJugador) {
          ...
          // ACA SE EJECUTA VER CUAL ES LA CONQUISTA MAS BARATA, YA SE LE MANDA UN TERRITORIO QUE SE SABE ES VALIDO, PERO EN LA FUNCION grafo.ConquistaMasBarata(terriAtacante) POR FAVOR DARLE LA OPCION DE ATAQUE MAS BARATA CON LAS PAISES QUE NO POSEE
          grafo.ConquistaMasBarata(it,terriDefensor); 
          bool validCountry = false;
          while (!validCountry) {
